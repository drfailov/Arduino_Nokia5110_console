...
const int sdinPin = 51; //pin 11 on UNO       pin 51 on MEGA        DN(MOSI) - Serial data, pin 6 on LCD.
const int sclkPin = 52; //pin 13 on UNO       pin 52 on MEGA        SCLK - Serial clock, pin 7 on LCD.
...
void pull_lines_up(){
  int line_height = 8;
  int total_bytes = (LCD_WIDTH * LCD_HEIGHT / 8);
  int offset = (line_height * LCD_WIDTH) / 8;
  /* pull data */
  for (int i=0; i < total_bytes-offset; i++)
  {
    displayMap[i] = displayMap[i+offset];
  }
  /* clear others */
  for (int i=total_bytes - offset; i < total_bytes; i++)
  {
    displayMap[i] = 0x00;
  }
}
...
{  
  ...
  LCDWrite(LCD_COMMAND, 0xB1 | contrast); //Set LCD Vop (Contrast): Try 0xB1(good @ 3.3V) or 0xBF if your display is too dark//0x80
